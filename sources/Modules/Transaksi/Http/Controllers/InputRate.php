...
class InputRate extends Controller
{
    ...
    {
        $getmapping = modelmappingratefcl::with(['country', 'polcity', 'podcity', 'shipping'])->where('id', $request->id)->where('aktif', 'Y')->select('id', 'id_country', 'id_polcity', 'id_podcity', 'id_shippingline', 'periodeawal', 'periodeakhir')->first();
        // dd($getmapping);
        $nama = Session::get('session')['user_nama'];
        $awal = Carbon::parse($getmapping->periodeawal)->format('d M');
        $akhir = Carbon::parse($getmapping->periodeakhir)->format('d M');
    ...
    public function add(Request $request)
    {
        ...
        $insert = modelinputratefcl::insert([
            'id_mappingrate' => $request->idmappingrate,
            'id_forwarder'   => $cekfwd->id,
            'of_20'          => $request->of20,
            'of_40'          => $request->of40,
            'of_40hc'        => $request->of40hc,
            'lb_20'          => $request->lb20,
            'lb_40'          => $request->lb40,
            'lb_40hc'        => $request->lb40hc,
            'aktif'          => 'Y',
